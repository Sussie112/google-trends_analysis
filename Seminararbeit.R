...
# ============================================================
# Data preparation: create vienna_tourism.csv
# ============================================================
...
file_airport <- "airport vienna.csv"
file_hotel   <- "hotel vienna.csv"
file_austria <- "vienna austria.csv"
file_hostel  <- "Vienna hostel.csv"
...
month_to_number <- function(month_name) {
  ...
  month_name <- tolower(trimws(month_name))
  
  months_map <- c(
    "january" = 1, "jänner" = 1, "jaenner" = 1, "januar" = 1,
  ...
  if (is.na(month_num)) {
    stop(paste("Unknown month name:", month_name))
  }
  
  return(as.numeric(month_num))
}


# ============================================================
# Function to read one monthly arrivals Excel file
# ============================================================

read_arrival_file <- function(file) {
  
  x <- read_excel(file, col_names = FALSE)
  
  month_name <- as.character(x[[1]][3])
  year_value <- as.numeric(x[[2]][3])
  arrivals_foreign <- as.numeric(x[[2]][9])
  
  month_num <- month_to_number(month_name)
  
  time_value <- as.Date(
    paste(year_value, month_num, "01", sep = "-")
  )
  
  out <- data.frame(
    Time = time_value,
    Arrivals = arrivals_foreign
  )
...
# ============================================================
# Read and combine all 48 monthly arrivals files
# ============================================================

arrival_files <- list.files(
  path = arrival_folder,
  pattern = "\\.xlsx$",
  full.names = TRUE
)

if (length(arrival_files) == 0) {
  stop("No Excel files found in the raw_arrivals folder.")
}

arrivals_all <- do.call(
  rbind,
  lapply(arrival_files, read_arrival_file)
)

arrivals_all <- arrivals_all[order(arrivals_all$Time), ]

arrivals_all <- subset(
  arrivals_all,
  Time >= as.Date("2022-01-01") & Time <= as.Date("2025-12-01")
)

print(arrivals_all)
cat("Number of monthly arrival observations:", nrow(arrivals_all), "\n")
...
read_google_file <- function(file, new_name) {
  ...
  raw <- raw[
    grepl("^\\d{4}-\\d{2}$", raw[[1]]) |
      grepl("^\\d{4}-\\d{2}-\\d{2}$", raw[[1]]),
  ]
  
  g <- raw[, 1:2]
  colnames(g) <- c("Time", new_name)
  
  # Convert Time
  g$Time <- ifelse(
    grepl("^\\d{4}-\\d{2}$", g$Time),
    paste0(g$Time, "-01"),
    g$Time
  )
  
  g$Time <- as.Date(g$Time)
  
  # Convert Google Trends values
  g[[new_name]] <- gsub("<1", "0.5", g[[new_name]])
  g[[new_name]] <- as.numeric(g[[new_name]])
  
  return(g)
}

airport <- read_google_file(file_airport, "airport.vienna")
hotel   <- read_google_file(file_hotel, "hotel.vienna")
austria <- read_google_file(file_austria, "vienna.austria")
hostel  <- read_google_file(file_hostel, "Vienna.hostel")

google_all <- Reduce(
  function(x, y) merge(x, y, by = "Time", all = FALSE),
  list(austria, hotel, airport, hostel)
)

google_all <- google_all[order(google_all$Time), ]
...
rownames(google_all) <- NULL

cat("Number of Google Trends observations:", nrow(google_all), "\n")
print(google_all)

# ============================================================
# Merge arrivals and Google Trends by month
# ============================================================

vienna_tourism <- merge(
  arrivals_all[, c("Time", "Arrivals")],
  google_all,
  by = "Time",
  all = FALSE
)

vienna_tourism <- vienna_tourism[order(vienna_tourism$Time), ]
rownames(vienna_tourism) <- NULL

vienna_tourism <- vienna_tourism[, c(
  "Time",
  "Arrivals",
  "vienna.austria",
  "hotel.vienna",
...
cat("Number of final observations:", nrow(vienna_tourism), "\n")
print(vienna_tourism)
str(vienna_tourism)
...
cat("Missing in arrivals:\n")
print(setdiff(all_months, arrivals_all$Time))

cat("Missing in Google Trends:\n")
print(setdiff(all_months, google_all$Time))

cat("Missing after merge:\n")
print(setdiff(all_months, vienna_tourism$Time))

if (nrow(vienna_tourism) != 48) {
  warning("The final dataset does not contain 48 observations. Please check the input files and date formats.")
}

if (any(is.na(vienna_tourism))) {
  warning("The final dataset contains missing values.")
}

# ============================================================
# Save final dataset
# ============================================================
colnames(vienna_tourism) <- c(
  "Time",
  "Arrivals",
  "vienna austria",
  "hotel vienna",
...
cat("Final dataset saved as vienna_tourism.csv\n")
